...
export default function AdoptModal({
  ...
  const isNameValid =
    nameLettersCount >= 2 && trimmedName.length <= 25;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Submit adoption inquiry to Formspree
      const response = await fetch(
        'https://formspree.io/f/mgaeqjdl',
        {
          method: 'POST',
          headers: {
      ...
      console.log('Formspree Adoption Response:', result);
      ...
      console.error(
        '[AdoptModal] Adoption inquiry submission failed:',
        err
      );
                      ...
                      <input
                        ...
                        className={`w-full bg-white dark:bg-slate-800 border ${
                          nameError
                            ...
                            : 'border-gray-200 dark:border-slate-700 focus:ring-amber-500'
                        } rounded-lg py-2.5 px-3 text-sm focus:outline-none focus:ring-1 text-gray-800 dark:text-slate-100`}
                      />

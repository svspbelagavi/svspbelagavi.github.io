...
export default function ContactSection({ lang }: ContactSectionProps) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    const lettersCount = trimmedSanitizedName.replace(
      /[^A-Za-z]/g,
      ''
    ).length;
    ...
    if (
      /[^A-Za-z ]/.test(trimmedSanitizedName) ||
    ...
    try {
      ...
      console.error('[ContactSection] Form submission failed:', err);
                    ...
                    <div>
                      ...
                      {emailError && (
                        <span
                          role="alert"
                          aria-live="polite"
                          className="block text-[10px] font-bold text-red-500 font-mono mt-1"
                        >
                          {emailError}
                        </span>
                      )}
